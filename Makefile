produce:
	...
	python3 produce.py
	sed -r 's#route ([0-9\.\/]+) via .*#/ip/route/add dst-address=\1 gateway=wg-ay routing-table=main distance=1 comment=imports#g;1i /ip/route/remove [find comments=imports]' routes4.conf > static-routes4.rsc
